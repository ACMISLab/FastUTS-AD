...
def normalize_score(pred_score):
    """
    Normalize the predict score.

    The nan and inf will be converted to zero.
    ...
    pred_score[np.isinf(pred_score)]=0
    if np.max(pred_score) == np.min(pred_score):
        det = 1
    else:
        det=np.max(pred_score)-np.min(pred_score)
    score = (pred_score-np.min(pred_score)) / det
    return score
