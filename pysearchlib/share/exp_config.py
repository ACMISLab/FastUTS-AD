...
from pysearchlib.utils.util_sys import get_gpu_device
from pysearchlib.utils.util_system import UtilSys, US
# from pysearchlib.share.dataset_loader import KFoldDatasetLoader, DataProcessingType
from pysearchlib.share.dataset_loader import KFoldDatasetLoader, DataProcessingType
...
def _load_data_set_v2(**kwargs):
    ...
    return dl.get_kfold_sliding_windows_train_and_test_from_fold_index()
...
class ExpConf:
    ...
    def dump(self):
        ...
        """
        assert self.fold_index is not None, "Fold index cannot be None"
    ...
    def load_data(self):
        """
        train_x, train_y, test_x, test_y=c.load_data()
        Returns
        -------
    ...
    def save_score_dth_exp_conf_to_npz(self, score: np.ndarray, dt: DateTimeHelper):
        if dt is None:
            dt = DateTimeHelper()
        if score is None:
            score = np.asarray([-1])
    ...
    def save_score_dth_exp_conf(self, score: np.ndarray, dt: DateTimeHelper):
        if self.is_save_to_redis():
            return self.save_score_dth_exp_conf_to_redis(score, dt)
        else:
            return self.save_score_dth_exp_conf_to_npz(score, dt)

    def save_score_dth_exp_conf_to_redis(self, score, dt: DateTimeHelper):
        if score is None:
            score = []
        score = MinMaxScaler(feature_range=(0, 1)).fit_transform(score.reshape(-1, 1)).ravel()
        score = np.round(score, 3)
    ...
    def _save_metrics_to_bz2(self, df):
        if UtilSys.is_debug_mode() and os.path.exists(self.get_metrics_file_name()):
            log.info(
                f"✅✅✅ Metrics has saved to file: {os.path.abspath(self.get_metrics_file_name())}\n")
        else:
            log.info(f"✅✅✅ Metrics has saved to file: {os.path.abspath(self.get_metrics_file_name())}\n")
...
def load_model(conf: ExpConf):
    if conf.model_name == "hbos":
        ...
        assert conf.is_semi_supervised_model() == False
        from benchmark_models.iforest.iforest import IForest
        clf = IForest()
    elif conf.model_name == "lof":
        assert conf.is_semi_supervised_model() == False
        from benchmark_models.lof.lof import LOF
        clf = LOF()
    elif conf.model_name == "ocsvm":
        assert conf.is_semi_supervised_model() == True
        from benchmark_models.ocsvm.ocsvm import OCSVM
        clf = OCSVM()
        ...
        # from benchmark_models.dagmm.dagmm_master.dagmm import DAGMM
        ...
        from benchmark_models.tadgan.tadgan_model import TadGanEd, TadGanEdConf
        cf = TadGanEdConf()
