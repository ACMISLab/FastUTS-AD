...
from pysearchlib.common import ConstTuner, Config, ConstNNI, Emjoi, SampleType
from pysearchlib.share.NNIExpResult import NNIExperimentUtils
from pysearchlib.util_dataset import DatasetName
from pysearchlib.utils.util_base64 import base64_encode_str
from pysearchlib.utils.util_datatime import get_datatime
from pysearchlib.utils.util_feishu import send_msg_to_feishu
from pysearchlib.utils.util_hash import get_str_hash
from pysearchlib.utils.util_json import is_json
from pysearchlib.utils.util_log import get_logger
from pysearchlib.utils.util_message import log_warn_msg, logi
from pysearchlib.utils.util_nni import get_available_nni_experiment_port, get_experiments_by_name
from pysearchlib.utils.util_nni_dispatcherlog import NNIDispatcherLog
from pysearchlib.utils.util_nni_exp_info import NNIExperimentInfo
from pysearchlib.utils.util_nni_managerlog import NNIManagerLog
from pysearchlib.utils.util_nni_sqlite import NNISqlite
from pysearchlib.utils.util_nnictl import NNICTL
...
class NNIExpGenerator:
    ...
    **Experiment parameter(generating experiments for nni) starts np_**,
    ...
    If you want to add a new parameter (--hpy 2) to the target model_entry_file, e.g., adding --hpy for main.py, you can
    ...
    def __init__(self,
        ...
        e.g., adding --hpy for main.py, you can NNIExpGenerator(_mp_hpy=2)
        ...
        entry_file: str
            The entry file for the experiment, e.g., main.py
        ...
        gpus: str
            The index of the GPU to use for training. Multi-GPUs can be used by comma separated.
        ...
        wait_interval_time: int
            The scan interval for query NNI results, i.e. the command: nnictl experiments list --all
    ...
    def __wait_exp_done_v2(self):
        if self._exp_info.get_exp_id() is None:
            msg = "Experiment is not found before waiting experiment done."
    ...
    def _get_model_parameter_key(self, _k):
        """
        If the _k is start with `_mp_`, then it's a model parameter, and return the key name. For example,
        `_mp_abc` will return "--abc".

        None if _k is not start with `_mp_`.
