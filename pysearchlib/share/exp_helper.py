...
from pysearchlib.share.exp_config import ExpConf, load_model
from pysearchlib.utils.util_bash import CMD, UtilBash
from pysearchlib.utils.util_common import UtilComm
from pysearchlib.utils.util_gpu import UGPU
from pysearchlib.utils.util_joblib import JLUtil
from pysearchlib.utils.util_log import get_logger, logconf
from pysearchlib.utils.util_number import str_to_float
from pysearchlib.utils.util_sys import get_gpu_count
from pysearchlib.utils.util_system import UtilSys
from pysearchlib.utils.util_tqdm import set_percent
from pysearchlib.share.dataset_loader import KFoldDatasetLoader, DatasetLoader, DataProcessingType
import warnings
...
def _print_end_job(exp_name):
    print(f"✅✅✅ All tasks for [{exp_name}] has done!")
...
def get_joblib_arg_hash(job: ExpConf):
    ...
    模型训练的缓存路径: /remote-home/acmis_fdd/joblib_cache/joblib/pysearchlib/experiments/exp_helper/_train_model_kfold/
...
def fsnotify(fun):
    def wrap(*args, **kwargs):
        ...
        if len(jobs) > 0:
            ...
            fun(*args, **kwargs)
            FSUtil.send_msg_to_feishu(f"✅All experiments for [{exp_name}] is done")
...
def train_models_kfold(conf: ExpConf):
    try:
        ...
        if conf.is_computed_score():
            UtilSys.is_debug_mode() and log.info(f"Metrics is existed at \n{conf.get_metrics_file_name()}\n")
            return None
    ...
    try:
        ...
        for _fold_index in range(conf.kfold):
            ...
            if conf.is_compute_metrics():
                tqdm.write(f"Metrics {conf.get_metrics_file_name()} is computed!")
                ...
                for _data_sample_method in args.data_sample_methods:
                    for _fold_index in range(args.kfold):
                        ...
                        if not _conf.is_compute_metrics():
                            ...
                            log.info(f"✅ Metrics file is exist at {_conf.get_metrics_file_name()}")
...
def main_cpu_v5(args):
    ...
    UtilSys.is_debug_mode() and log.info("✅✅✅ Tasks done!")
    FSUtil.send_msg_to_feishu(f"✅All experiments for [{args.exp_name}] is done")
...
def run_job_on_gpu(array, gpu_index):
    while len(array) > 0:
        ...
        if conf.is_computed_score():
            log.info(f"✅ Job with index [{conf.exp_index}] is finished")
        else:
...
def _print_model_metrics(conf, metrics):
    if UtilSys.is_debug_mode() or UtilSys.is_macos():
        log.info(
            f"✅✅✅ Model [{conf.model_name}]({conf.exp_name}) trained done. Metrics:\n{metrics}\n")
