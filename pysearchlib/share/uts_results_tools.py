...
from pysearchlib.share.ExcelOutKeys import ExcelMetricsKeys, EK, PaperFormat
from pysearchlib.share.gather_metrics import FileGathers
from pysearchlib.share.exp_helper import JobConfV1
from pysearchlib.utils.util_bash import CMD
...
from pysearchlib.utils.util_file import FileUtil, FileUtils
from pysearchlib.utils.util_gnuplot import _generate_xlabel, UTSViewGnuplot
from pysearchlib.utils.util_joblib import JLUtil
from pysearchlib.utils.util_log import get_logger
from pysearchlib.utils.util_number import round_dict
from pysearchlib.utils.util_pandas import PDUtil
from pysearchlib.utils.util_statistical import UtilHypo
from pysearchlib.utils.util_sys import get_num_cpus
...
def is_stopped_training_v2(metrics, post_see, alpha=0.001):
    ...
    UtilSys.is_debug_mode() and log.info(
        f"Decide whether to stop. Cur value: {indexer}, is stop: {indexer < alpha}, post see={post_see}, stop alpha={alpha}, Metrics: \n{metrics}")
    if indexer < alpha:
        return True
    else:
        return False
...
class ExcelMetricAnalysis:
    ...
    def merge_kfold_df(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # pd.crosstab(index=ExcelMetricAnalysis.GROUP_BY_KEYS, columns=[EK.VUS_ROC, EK.VUS_PR, EK.RF], values=[''])
        # remove debug data
