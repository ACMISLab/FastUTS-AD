...
from pysearchlib.common import ConstNNI, ConstMetric, Emjoi
from pysearchlib.util_pandas import show_all_pandas
from pysearchlib.utils.util_file import generate_random_file
...
class NNIExperimentUtils:
    ...
    def _get_exp_running_seconds(self, exp_id):
        ...
        c.execute(
            "SELECT trialJobId,max(timestamp) as end,min(timestamp) as start,(max(timestamp)-min(timestamp))/1000 as elapse from TrialJobEvent WHERE event !='WAITING' GROUP BY trialJobId ")
        record = c.fetchall()
