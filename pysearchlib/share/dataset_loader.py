...
from pysearchlib.common import SampleType
from pysearchlib.share.ExcelOutKeys import EK
from pysearchlib.utils.util_file import FileUtils, FileUtil
from pysearchlib.utils.util_joblib import JLUtil
from pysearchlib.utils.util_log import get_logger, logconf
from pysearchlib.utils.util_message import logw
from pysearchlib.utils.util_numpy import feature_, enable_numpy_reproduce
from pysearchlib.utils.util_pandas import butify_print_options, PDUtil
from pysearchlib.utils.util_sampling import latin_hypercube_sampling
from pysearchlib.utils.util_system import UtilSys
from pysearchlib.utils.util_univariate_time_series import subsequences
from pysearchlib.utils.utils import convert_str_to_float

log = get_logger()

memory = JLUtil.get_memory()

butify_print_options()
...
def post_process_sliding_window_label(label_sliding_window, anomaly_window_type="all", time_step=None):
    if anomaly_window_type == AnomalyWindowType.ALL:
        ...
        UtilSys.is_debug_mode() and logconf("Anomaly windows type: all")
        ...
        # https://uniplore.feishu.cn/wiki/wikcnCDt1FT72TcwAkXOPTao8Ec?sheet=EAgI0Q
        #
        UtilSys.is_debug_mode() and logconf("Anomaly windows type: coca")
...
def get_all_available_datasets(max_size_mb=10):
    ...
    for f in files:
        ...
        if f_size > max_size_mb:  # >10m
            UtilSys.is_debug_mode() and warnings.warn(f"File {f} is to large in our experiments, up to {f_size}m ")
            continue
...
class DatasetLoader:
    ...
    def get_data_id(self, data_index) -> DataItem:
        ...
        assert data_index < all_data.shape[
            0], f"The dataset index = {data_index} is out of range. except [0 ~ {all_data.shape[0] - 1}]"
...
class KFoldDatasetLoader:
    ...
    def get_dataset_home():
        ...
        assert home is not None and os.path.isdir(
            home), "Home of dataset must be specified by Environment Variable DATASET_HOME, such as export DATASET_HOME=/Users/sunwu/Documents/uts_benchmark_dataset/benchmark/ \n Under the benchmark directory, it contains " \
    ...
    def __init__(self,
        ...
        # UtilSys.is_debug_mode() and logconf(f"Using the default dataset home {self._dataset_home}")
    ...
    def get_sliding_windows_with_origin_y(self):
        ...
        if self._processing == "normal":
            logconf("Normalizing data ...")
            values = data.iloc[:, 0]
            data.iloc[:, 0] = (values - values.min()) / (values.max() - values.min())
        elif self._processing == "standardization":
            logconf("Standardize data ...")
            values = data.iloc[:, 0]
            data.iloc[:, 0] = (values - values.mean()) / (values.std())
        else:
            warnings.warn("Skip pre-processing input data ...")
    ...
    def get_sliding_windows(self, return_origin=False):
        ...
        self._data_processing_time = _end_time - _start_time
        if convert_str_to_float(self._sampling_rate) == 0:
            X = np.zeros((1, X.shape[1]))
            labels = np.asarray([1])
    ...
    def get_kfold_sliding_windows_train_and_test_by_fold_index(self, fold_index=-1):
        ...
        """
        # UtilSys.is_debug_mode() and logconf(f"Dataloader Config: \n{pprint.pformat(self.__dict__)}")
        if fold_index < 0:
    ...
    def get_kfold_sliding_windows_train_and_test_from_fold_index(self):
        ...
        """
        # UtilSys.is_debug_mode() and logconf(f"Dataloader Config: \n{pprint.pformat(self.__dict__)}")
        # assert self._fold_index is not None, "k-fold index cannot be None."
    ...
    def _data_sampling(self, train_x, labels):
        ...
        if self._data_scale_beta is not None and self._data_scale_beta > 0:
            ...
            UtilSys.is_debug_mode() and logconf("Data size auto scale is disabled")
        # convert to n_samples
        ...
        if self._data_sampling_method == SampleType.RANDOM:
            ...
            if n_all == n_samples:
                ...
                UtilSys.is_debug_mode() and log.info(
                    f"Sampling method is LHS with sampling index shape: {len(sampled_index)}, first 10:\n{sampled_index[:10]}")
            ...
            if n_samples <= 0:
                ...
                UtilSys.is_debug_mode() and log.info(
                    f"Sampling method is {self._data_sampling_method} with sampling index shape: {len(_all_sampled_index)}, first 10:\n{_all_sampled_index[:10]}")
