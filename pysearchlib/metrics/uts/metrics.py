...
class UtilMetrics:
    ...
    def metric_new(self, label, score, plot_ROC=False, alpha=0.2, coeff=3):
        ...
        '''
        if np.sum(label) == 0:
            print('All labels are 0. Label must have ground truth value for calculating AUC score.')
            return None
    ...
    def RangeAUC_parallel(self, labels, score, window=0, percentage=0, plot_ROC=False, AUC_type='window'):
        # AUC_type='window'/'percentage'
        score_sorted = -np.sort(-score)
        ...
        result = Parallel(n_jobs=-1)(
            delayed(_calculate)(i) for i in np.linspace(0, len(score) - 1, 250).astype(int))
        ...
        TPR_list = TPR_list + list(result[:, 0])
        FPR_list = FPR_list + list(result[:, 1])
        Precision_list = Precision_list + list(result[:, 2])
        TPR_list.append(1)
        FPR_list.append(1)  # otherwise, range-AUC will stop earlier than (1,1)
...
def generate_curve(label, score, slidingWindow):
    tpr_3d, fpr_3d, prec_3d, window_3d, avg_auc_3d, avg_ap_3d = UtilMetrics().RangeAUC_volume(labels_original=label,
                                                                                              score=score,
...
def get_metrics(score, labels, metric='all', window_size=None):
    ...
    if metric == 'vus':
        ...
        AUC_ROC, Precision, Recall, F, Rrecall, ExistenceReward, OverlapReward, Rprecision, RF, Precision_at_k = grader.metric_new(
            labels, score, plot_ROC=False)
...
@memory.cache
def get_metrics_cacheable(score, labels, metric='all', window_size=None):
    slidingWindow = window_size
    ...
    if metric == 'vus':
        ...
        AUC_ROC, Precision, Recall, F, Rrecall, ExistenceReward, OverlapReward, Rprecision, RF, Precision_at_k = grader.metric_new(
            labels, score, plot_ROC=False)
        _, _, AUC_PR = grader.metric_PR(labels, score)
        metrics = {'Precision': Precision, 'Recall': Recall, 'F': F, 'AUC_ROC': AUC_ROC, 'AUC_PR': AUC_PR,
                   'Precision_at_k': Precision_at_k, 'Rprecision': Rprecision, 'Rrecall': Rrecall, 'RF': RF,
                   'R_AUC_ROC': R_AUC_ROC, 'R_AUC_PR': R_AUC_PR, 'VUS_ROC': VUS_ROC, 'VUS_PR': VUS_PR}
...
def get_metrics_parallel(score, labels, metric='all', slidingWindow=None):
    assert slidingWindow is not None, "slidingWindow must be specified"
    metrics = {}
    if metric == 'vus':
        grader = UtilMetrics()
        R_AUC_ROC, R_AUC_PR, _, _, _ = grader.RangeAUC_parallel(labels=labels, score=score, window=slidingWindow,
                                                                plot_ROC=True)
        _, _, _, _, _, _, VUS_ROC, VUS_PR = generate_curve(labels, score, 2 * slidingWindow)
