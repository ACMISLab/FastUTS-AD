...
from pysearchlib.ts_datasets.base import BaseDataset, _main_fns_docstr
# from ..base import BaseDataset, _main_fns_docstr

_intro_docstr = """
Base m_dataset class for storing time series intended for anomaly detection.
"""
...
class TSADBaseDataset(BaseDataset):
    ...
    def max_lead_sec(self):
        ...
        this m_dataset. ``None`` signifies no early detections allowed, or that
    ...
    def describe(self):
        ...
        print("=" * 80)
        print(f"Time series in m_dataset have average length {int(np.mean([len(ts) for ts, md in self]))}.")
        print(f"Time series in m_dataset have {len(anom_bds) / len(self):.1f} anomalies on average.")
        print(
            f"{sum(anom_in_trainval) / len(anom_in_trainval) * 100:.1f}% of "
            f"anomalies are in the train/val split of their respective time "
            f"series."
        )
        print(f"Anomalies in m_dataset have average length {pd.Timedelta(np.mean([(tf - t0) for t0, tf in anom_bds]))}.")
        print(
