...
from pysearchlib.ts_datasets.anomaly.base import TSADBaseDataset
...
class SMD(TSADBaseDataset):
    """
    The Server Machine Dataset (SMD) is a new 5-week-long m_dataset from
    ...
    def __init__(self, subset="all", rootdir=None):
        ...
        # Download the SMD m_dataset if it doesn't exist
...
def combine_train_test_datasets(train_df, test_df, test_labels):
    ...
    # The m_dataset is only for unsupervised time series anomaly detection
...
def download(logger, datapath, url, filename):
    ...
    # Download the compressed m_dataset
