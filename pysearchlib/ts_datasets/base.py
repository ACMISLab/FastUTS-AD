...
_intro_docstr = "Base m_dataset class for storing time series as ``pd.DataFrame`` s."

_main_fns_docstr = """
Each m_dataset supports the following features:

1.  ``__getitem__``: you may call ``ts, metadata = m_dataset[i]``. ``ts`` is a time-indexed ``pandas`` DataFrame, with
    each column representing a different variable (in the case of multivariate time series). ``metadata`` is a dict or
    ``pd.DataFrame`` with the same index as ``ts``, with different keys indicating different m_dataset-specific
    metadata (train/test split, anomaly labels, etc.) for each timestamp.
2.  ``__len__``:  Calling ``len(m_dataset)`` will return the number of time series in the m_dataset.
3.  ``__iter__``: You may iterate over the ``pandas`` representations of the time series in the m_dataset with
    ``for ts, metadata in m_dataset: ...``
...
class BaseDataset:
    ...
    time_series: list
    """
    A list of all individual time series contained in the m_dataset. Iterating over
    the m_dataset will iterate over this list. Note that for some large datasets, 
    ...
    metadata: list
    """
    A list containing the metadata for all individual time series in the m_dataset.
    """
