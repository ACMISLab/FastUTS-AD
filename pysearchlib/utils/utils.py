...
log = get_logger()
from pysearchlib.utils.util_network import get_random_idle_port as idle_port
...
def report_final_result_to_console(best_f_scores, aucs):
    print(f"=========== Final result ========="
          f"\nAuc: {np.round(np.mean(aucs) * 100, 4)},"
          f"\nbest F-Score: {np.round(np.mean(best_f_scores) * 100, 4)}")


def report_intermediate_result_to_console(k_fold_index, auc, best_f_scores, best_precsion=None, best_recall=None):
    print(f"------------ Intermediate result ------------"
          f"\nFold index: {k_fold_index}, "
          f"\nAuc: {auc}, "
          f"\nbest F-Score {best_f_scores}"
