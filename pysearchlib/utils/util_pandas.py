...
from pysearchlib.utils.util_datatime import DateTimeFormat
...
from pysearchlib.utils.util_number import is_number
...
def save_pandas_to_mysql(df: pd.DataFrame, table_name="data", uri: str = None):
    """
    Save pa.DataFrame to mysql specified by URI.
    ...
    if df is None:
        log.warning("Df is not available for None table")
    else:
        # Convert all headers to upper case
        # fix: sqlalchemy.exc.OperationalError: (pymysql.err.OperationalError) (1060, "Duplicate column name 'A'")
        df.columns = df.columns.map(lambda a: a.upper())

        if "TIMESTAMP" in df.columns:
            df = df.set_index("TIMESTAMP", )
        try:
            row_affected = df.to_sql(table_name, engine, if_exists='replace', index=True, dtype=None)
        except:
            row_affected = df.to_sql(table_name, engine, if_exists='replace', index=True, dtype=None)

        UtilSys.is_debug_mode() and log.info(f"Saving {row_affected} records to {table_name}")
...
def save_pandas_to_sqlite(df: pd.DataFrame,
    ...
    from pysearchlib.utils.util_file import make_dirs
    make_dirs(home)
...
def print_pretty_table(df: pandas.DataFrame, name=""):
    ...
    pretty_df = tabulate(df, tablefmt='psql', showindex=True, maxcolwidths=30,
                         floatfmt='.2f')
    log.info(f"{name}\n {pretty_df}")
...
class PDUtil:
    ...
    @staticmethod
    def save_to_excel(df: pd.DataFrame, name=UC.get_filename_entry(),
                      home=UtilComm.get_entrance_directory(),
        ...
        if append_entry:
            ...
            file_name = os.path.join(home, f"{prefix}_{name}.xlsx")
        else:
            file_name = os.path.join(home, f"{name}.xlsx")
        file_name = os.path.abspath(file_name)
        UtilSys.is_debug_mode() and log.info(f"Saved [{os.path.basename(file_name)}] to {file_name}\n")
        if os.path.exists(file_name):
            warnings.warn(f"File is already exists. {file_name}")
        df.to_excel(file_name, index_label=index_label, index=index)
        return file_name

    @staticmethod
    def save_to_csv(df: pd.DataFrame, name, home=UtilComm.get_runtime_directory(), index=True, index_label='index',
                    append_entry=False):
        """
        保存为文件
        ...
        log.info(f"✅ Saved metrics to {os.path.abspath(file_name)}")
    ...
    @classmethod
    def print_pretty_table_with_header(cls, df: pd.DataFrame):
        pretty_df = tabulate(df,
                             tablefmt='psql',
                             showindex=True,
                             maxcolwidths=30,
                             headers=df.columns,
