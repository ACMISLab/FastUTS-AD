...
@typechecked
def is_normal_distribution(data: np.ndarray, alpha=0.01, check_type="shapiro", return_pvalue: bool = True):
    """
...
def is_significant_decrease(baseline_metrics: np.ndarray, sampled_metrics: np.ndarray, alpha: float = 0.01,
                            return_t_and_p: bool = False):
    """
    Return:
    is_sig_decrease, t, p = is_significant_decrease(baseline_metrics, sampled_metrics)  if return_t_and_p=True
...
if __name__ == '__main__':
    ...
    assert is_normal_distribution(np.random.uniform(1, 10, 100)) == False
    assert is_normal_distribution(np.random.normal(size=100)) == True
