...
class PCA(DetectorB):
    ...
    def score(self, X):
        ...
        # convert the nan to -1
        w_ = self.selected_w_components_
        w_[np.isnan(w_)] = -1
        w_[w_ == 0] = 1e-20
        return np.sum(cdist(X, self.selected_components_) / w_, axis=1).ravel()
        # return np.sum(
    ...
    def fit(self, X, y=None):
        ...
        self.detector_ = _PCA(n_components=self.n_components,
                              copy=self.copy,
                              whiten=self.whiten,
    ...
    def get_parameters_desc(self):
        return np.mean(self.noise_variance_) + np.mean(self.singular_values_) + np.mean(
            self.explained_variance_) + np.mean(self.n_components_)
