...
"""
from pysearchlib.utils.util_class import initializer
from pysearchlib.utils.util_pytorch import convert_to_dl
...
class AutoEncoder:
    ...
    def fit(self, X, y=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, (batch, _) in enumerate(train_loader):
                # train batch.shape (37, 64, 1)
                batch = torch.unsqueeze(batch.squeeze(dim=1), 2).float().to(self.device)
                ...
                optimizer.step()
                total_loss += loss
                UtilSys.is_debug_mode() and log.info("Epoch {}, loss {}".format(epoch, loss))
    ...
    def score(self, X):
        ...
        for y, _ in data_loader:
            y = torch.unsqueeze(y.squeeze(dim=1), 2).float().to(self.device)
            # y = torch.tensor(y, dtype=torch.float, device=self.device)
            scores.append(self.model(y))
        # scores = np.concatenate([np.ones(self.k - 1) * scores[0][0], *scores])
        return torch.concat(scores, dim=0).cpu().detach().numpy()
