...
class DAGMM:
    ...
    def fit(self, X, y=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            for input_data, _, in train_loader:
                ...
                input_data = torch.unsqueeze(torch.squeeze(input_data).float().to(self.device), 2)

                # input_data=input_data..float(). to(self.device)
                loss, _, error, _ = self._step(input_data)
                total_loss += loss
                recon_error += error
                UtilSys.is_debug_mode() and log.info("epoch %s, loss %s" % (epoch, float(loss)))
    ...
    def score(self, X):
        ...
        for i, (sequence, _) in enumerate(data_loader):
            sequence = torch.unsqueeze(torch.squeeze(sequence).float().to(self.device), 2)
