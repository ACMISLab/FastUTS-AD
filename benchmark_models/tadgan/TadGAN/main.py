...
def train(n_epochs=2000):
    ...
    for epoch in range(n_epochs):
        UtilSys.is_debug_mode() and logging.debug('Epoch {}'.format(epoch))
        n_critics = 5
        ...
        UtilSys.is_debug_mode() and logging.debug('Critic training done in epoch {}'.format(epoch))
        encoder_loss = list()
        decoder_loss = list()
        ...
        # UtilSys.is_debug_mode() and logging.debug('Encoder decoder training done in epoch {}'.format(epoch))
        # UtilSys.is_debug_mode() and logging.debug('critic x loss {:.3f} critic z loss {:.3f} \nencoder loss {:.3f} decoder loss {:.3f}\n'.format(cx_epoch_loss[-1], cz_epoch_loss[-1], encoder_epoch_loss[-1], decoder_epoch_loss[-1]))
