...
class VAEED:
    ...
    def fit(self, X, y=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch, _ in train_loader:
                # batch.shape=(1024, 64)
                x = torch.squeeze(batch, dim=1).float().to(self.device)
    ...
    def score(self, X) -> pd.DataFrame:
        ...
        for y, _ in data_loader:
            # y.shape=(1024, 64)
            y = torch.squeeze(y, dim=1).to(self.device)
            ys.append(y)
            # r = np.zeros(y.shape)
            r = torch.zeros(y.shape).to(self.device)
        ...
        test_scores = torch.sum(torch.abs(torch.concat(rs) - torch.concat(ys)), dim=1)
        # scores[self.k - 1 :] = test_scores
        # scores[: self.k - 1] = test_scores[0]
        return test_scores.cpu().detach().numpy()
