...
from benchmark_models.tstcc.cpc_model import TSTCCConf, TSTCCModel
...
for _sr in JobConfV1.SAMPLE_RATES_DEBUGS:
    ...
    loader = ExpConf(dataset_name="ECG",data_id="MBA_ECG14046_data_46.out",
                     data_sample_rate=_sr,
                     fold_index=1,
                     window_size=conf.window_size,kfold=2)
    train_x, train_y, test_x, test_y = loader.load_dataset_at_fold_k()

    cuda = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Cuda: {cuda}")

    clf = TSTCCModel(conf, cuda)
    clf.fit(train_x)
