...
class affiliation_accumulator:
    ...
    def get_all_metrics_enchanced(self):
        p = self.precision / self.cnt
        r = self.recall / self.cnt
        f1 = 0.0 if p == 0 or r == 0 else 2 * p * r / (p + r)
        return {
            'precision': p,
            'recall': r,
            'f1': f1
    ...
    def get_all_metrics(self):
        # return self.get_all_metrics_source()
        return self.get_all_metrics_enchanced()

    def get_all_metrics_source(self):
        p = self.precision / self.cnt
        r = self.recall / self.cnt
        f1 = 0.0 if p == 0 or r == 0 else 2 * p * r / (p + r)
        return {
...
def tsad_affiliation(
    ...
    try:
        ...
        result = pr_from_events(events_pred, events_gt,
                                Trange=(0, max([max(x) for x in events_pred] + [max(x) for x in events_gt])))
        result['cnt'] = 1
