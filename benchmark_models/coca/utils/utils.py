...
from pysearchlib.ts_datasets.anomaly import *
...
def evaluate_predictions(
    ...
    for i, (true, md) in enumerate(tqdm(dataset)):
        ...
        for acc_id, (simple_threshold, opt_metric, scores) in enumerate(
            ...
            # Get the lead & lag time for the m_dataset
            ...
            # For UCR m_dataset, the evaluation just checks whether the point with the highest
            ...
            # Make sure all time series have exactly one detection for UCR m_dataset (either 1 TP, or 1 FN & 1 FP).
            if acc_id == 0 and use_ucr_eval:
    ...
    # Aggregate statistics from full m_dataset
...
def get_model(
    ...
    # Load the model with default kwargs, but override with m_dataset-specific
    ...
    # The post-rule train configs are fully specified for each m_dataset (where
    # relevant), with a default option if there is no m_dataset-specific option.
...
def read_model_predictions(dataset: TSADBaseDataset, model_dir: str):
    ...
    anomaly scores for time series i in the m_dataset.
    """
