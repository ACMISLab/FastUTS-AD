    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param sampling_mode: how to process time series data for the tree model. If "normal",
            ...
            measures over the window. Note: "stats" mode is statistical summary for a multivariate m_dataset,
...
class BaggingTreeForecaster(ForecasterBase, MultiVariateAutoRegressionMixin):
    ...
    def train(self, train_data: TimeSeries, train_config=None):
        ...
        if self.dim == 1:
            logger.info(
                f"Model is working on a univariate m_dataset, "
            ...
            if self.sampling_mode != "normal":
                logger.warning('For univariate m_dataset, only supports "normal" sampling mode')
        ...
        # multivariate case, sequence or 1-step autoregression
            ...
            if self.prediction_stride > 1:
                ...
                if self.prediction_stride != self.max_forecast_steps:
                    logger.warning(
                        f"For multivariate m_dataset, reset prediction_stride = max_forecast_steps = {self.max_forecast_steps} "
                    )
...
class RandomForestForecaster(BaggingTreeForecaster):
    ...
    trees on various sub-samples of the m_dataset, and uses averaging to improve
...
class ExtraTreesForecaster(BaggingTreeForecaster):
    ...
    the m_dataset and uses averaging to improve the predictive accuracy and
